...
void HttpConnection::createResponseGet(std::string no_results)
{
	if (request_.target() == "/")
	{
...
void HttpConnection::createResponsePost()
{
	if (request_.target() == "/")
	{
		...
		ini.read_property();
		ServerDataBaseWorker server_db_worker{ini};

		std::vector<std::string> search_result{server_db_worker.search_pages(utf8value)};
		if (search_result.size() == 0)
		{
			createResponseGet("No results");
			return;
		}
